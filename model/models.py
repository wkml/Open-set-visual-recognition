...
class SSGRL(nn.Module):
    def __init__(self, args, adjacency_matrix, word_features,
                image_feature_dim=2048, output_dim=2048, time_step=3,
                num_classes=80, word_feature_dim = 300):
    ...
    def forward(self, x):
        batch_size = x.size()[0]
        # 特征抽取
        img_feature_map = self.clip_model.encode_image(x)
        # 特征融合（SD）
